...
new = """        try:
            _recov_k = float(os.environ.get("S10_FP_RECOV_K", "15.0"))
            _recov_kq = float(os.environ.get("S10_FP_RECOV_KQ", "60.0"))
            _recov_on = False
            ...
            for _leg in range(4):
                ...
                _relx_r = (np.cos(body["yaw"])
                           ...
                           * (wheel_xyz[_leg, 1] - _hip_r[1]))
                _drop_r = float(_hip_r[2] - wheel_xyz[_leg, 2])
                # v1018: 折叠判定——支撑腿 relx<0/垂距<3cm；SWING 腿仅当
                # 轮高于几何目标+5cm（真过伸）时触发，解除折叠后再由阻抗
                # 保持。v1011 跳过 swing 腿导致过伸无对抗（轮 1.1 实测）。
