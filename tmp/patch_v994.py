...
old = """                _rel = np.array([np.cos(yaw) * (wheel_xyz[leg, 0] - _hip_w[0])
                                 + np.sin(yaw) * (wheel_xyz[leg, 1] - _hip_w[1]),
                                 _wz_t - _hip_w[2]])"""
new = """                _rel = np.array([np.cos(yaw) * (wheel_xyz[leg, 0] - _hip_w[0])
                                 + np.sin(yaw) * (wheel_xyz[leg, 1] - _hip_w[1]),
                                 _wz_t - _hip_w[2]])
                # v994: 悬空轮"收腿"——轮已抬到目标高度但未过棱(fn=0)时，
                # 腿把轮前推(relx 0.15+)的反作用把 body 往后拉，后轮推力被
                # 抵消、狗死锁 8s(实测 fn=0 + 后轮 6-13.5Nm 前驱推不动)。
                # 目标 relx 收到 0.06(轮收在髋下)，后轮推着 body 前进，悬空
                # 轮自然漂过棱后落到台面。
                if float(wheel_xyz[leg, 2]) > float(terrain_h[leg]) + self.fk.r + 0.03:
