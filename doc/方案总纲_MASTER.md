...
| 2026-08-16 | 592c3a4+ | C++ mesh 接触进一步确认：简单 RL PD/巡航式下压推近均无法建立 mesh 接触（轮子恒落地板 z=0.082），解算器 cone/jacobian 无关——**必须集成完整 CarVMC 控制**（腿阻抗+打滑检测+地形前瞻）才能在 mesh 上行驶（集成流爬至 38.7 即靠此）。C++ mesh 训练 env 需把 CarVMC 集成进训练循环（重大任务）。MJX 精炼 BEST 0.335、box 12/12 爬完 6 级/0 摔。 |
| 2026-08-16 | 29aae94+ | C++ mesh 接触机制确认：静态 settle（站姿/下压半蹲均试）无法建立 mesh 接触（轮子恒落地板 z=0.082），必须复刻巡航的"滚动+腿向下接触力"模式——集成流（cruise 驱动）能爬 mesh 至 y=38.7 正是靠此。C++ mesh 训练 env 需重构为滚动起步 + cruise 式接触建立（重大任务）。MJX 精炼 lr 3e-4：box 达标率提升至 12/12 爬完 6 级/0 摔（精炼前 10/12），真实 mesh 集成流仍 y≈38.7 yaw 自旋（mesh gap）。wp4→5 完整机制链已文档化（转向方向翻转→物理不可达→执行层打滑→抬轮误触发），限速 2.0 改善对准但仍卡（执行层需重构）。 |
...
| 2026-08-16 | 4a09d2f+ | **重大更正 + 交接修复**：① 官方 env 旧 confirm_official 在 x=0 平地 spawn——"29/30/96.7%/100% 官方验收"全部是**平地/趴地假象**：真实走廊(x=-14.4) spawn 后机身塌到 z=0.27-0.44 滑到 y=41.3（crawl 算 reach），而 box 训练地形同策略 z=0.84-0.98 **直立爬**（图 doc/figures/box_vs_real_z.png）——**sim2sim 迁移（MJX box→真实 mesh）才是核心瓶颈**（§3.36 轮驱 2.7× gap + mesh 接触）。② 集成交接根因链修复：riser 顶高改已知表（auto_nav STAIR_TOPS）；carvmc pose_target 过渡腿不到位(max leg_err 0.41-0.51)→改 **carvmc+PD 腿覆盖过渡**（leg_err→0.122，kp60/kd4 锁 default_dof，轮保持速度/yaw 不停车）；STAIR 已知地图提前触发(S10_STAIR_ENTER_DIST=5→y≈34.5)；交接状态诊断：[RL-DIAG] 打印 leg_err/yaw/vx。③ 用户流程确认：cruise 带速度角度接近→carvmc+PD 抬身→RL 直接接管爬梯（不刹 0，反流程已废弃）。④ T6_handoff 初始姿态 DR 训练启动（恢复自 model_000400_75pct_keep 100% base；yaw ±0.3 回退 ±1.0 防回归 + squat_frac 0.35 + leg_q_jit 0.25，真实 mesh 容忍度：蹲姿 0/30、jitter 0.3→70%、0.5→30%）。⑤ xyz-t 图 doc/figures/handoff28_xyz_t.png（集成流：cruise→过渡→RL 爬至 38.3 过首级→yaw 自旋摔倒）。 |
