...
- N768/NDIFFUSE=4（原配置）：plan 中位 73~79ms → 实际 13.2~14.0Hz（名义
  plan_interval13=15.4Hz）；scan≈50ms、upd≈20ms（含上一轮 GPU 同步等待）。
- N640/N512/NDIFFUSE 2-3 变体未提速（本机共享 GPU 争抢严重，重编译后
  scan 反升）；未找到可靠的 15Hz 达标配置。下一步方向：update_state 去 D2H
  同步（pose/z 目标改主机 q 计算，已验证语法，未验证前暂不回退代码）。
