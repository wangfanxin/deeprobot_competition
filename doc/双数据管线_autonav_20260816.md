...
### 3.5 交接验证与 wp5→6 减速结论（2026-08-16 实测）

**已验证的 wp5→9 集成流（确定性，同配置两次运行逐秒一致）**：
`wp5@1.16s → wp6@7.39s → wp7@14.99s → wp8@28.10s → wp9@32.96s`，完成 True（wp=10/10）。
RL 接管状态（RL-DIAG）：pos=(-14.43,34.44) yaw=1.471(84°) vx=2.27 max_leg_err=0.122；
RL→CRUISE 交接：pos=(-14.00,40.57) vx=1.26。有头验证命令：`run_wp59_viewer.sh`
（S10_AUTO_VMAX=3.5 + S10_VMC_USE_NAV=1 直连导航 + S10_ELEV_KNOWN_RAMP=2.0 +
PRETRANS 抬身 + STAIR_EXIT_VX=1.5；**不是 v890 MPPI env**）。

**交接机制（即"过 wp5 后同时调整 yaw 和 v"）**：wp6→7 段的**已知 stair-zone
邻近减速**（S10_ELEV_KNOWN_RAMP=2.0，沿 wp6→7 段 1m 起 5m 内 ramp decel_request）
把 v 从 3.5 平滑降到接管点 ~2.27；导航航向自然收敛到楼梯垂线（84°）；PRETRANS
(y=31→33) 用 carvmc+PD 覆盖抬身使 leg_err→0.12。三者叠加即"慢速+对准+标准站姿
接管"。**真实 mesh 上 RL 需要 ~2 m/s 接管动量**（实测 0.5/1.5/1.8 首级即摔，
2.27 爬完 6 级）——"速度几乎减到 0 再接管"会必然摔，不能执行。

**为什么 wp5 前没有高程图减速（用户提问）**：默认门控 S10_ELEV_MIN_STEPS≥2 +
S10_ELEV_CLIMB_TH≥0.4 故意忽略单/双级台阶（wp4→5 1 级 y=18.4；wp5→6 2 级
y=26.5/28.5），且 wp4→5 发卡弯需要动量（此前已验证减速致 MPPI 转向不足）。
降门控（MIN_STEPS=1, CLIMB_TH=0.10, DECEL_VX=2.5）实测：**lidar 只抓住第 1 级、
丢失第 2 级**（台阶间 ad=None，vref 提前回到 3.5 顶在第 2 级上）→ wp6 后 yaw
失控东/西偏出走廊，RL 在 yaw=142° 坏状态接管卡死。96 线高程图过稀，无法可靠
门控 wp5→6 双级台阶。

**已知图 wp5→6 减速的尝试（已回退）**：在 auto_nav 加 S10_ELEV_STEP_DECEL=1
（y∈[23.5,29.5] 已知图减速到 2.5 并保持过两级）——接管状态干净（yaw=79°/
vx=2.25/leg_err=0.14），但 RL 爬完在 y=40.74 以 vx=0.22 交回 → 平台侧翻
（两次运行逐秒一致，系统性非方差）。**结论：任何对 wp5→6 的提前减速都会扰动
接管状态，破坏已验证的 RL 爬梯-交接闭环；保持默认（台阶 3.5m/s 直过），
维持当前唯一稳定配置。**
## 4. 两管线对比
