...
> 技能切换（区域判定 / 进梯 / 出梯回巡航）：见 [stair_区域判定与技能切换计划_20260811.md](stair_区域判定与技能切换计划_20260811.md)。
...
- **显式轮心 z 参考**：`wheel_z_ref(y) = 台面顶高(y) + R`（R = 0.081m，与模型 FK 实测一致，已确认）。棱前 RAMP_A 平滑过渡，棱后贴面。
...
| 参数 | 终版方案 | 代码/模型实测 | 结论 |
|---|---|---|---|
| 轮半径 R | 0.081m（已确认） | **0.081m**（`vmc_legs.py` FK：L1=0.18、L2=0.18、r=0.081） | ✓ 公式与模型一致 |
| 轴距 | 0.456m | 0.456m | ✓ |
