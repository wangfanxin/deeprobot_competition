...
export S10_MPPI_ADA=1 S10_MPPI_A_MAX=3.5 S10_MPPI_OMAX=2.5 S10_MPPI_W_GUIDE=1.5 S10_MPPI_W_DIST=2.0 S10_MPPI_W_HEAD=2.0
# 终点代价：到 wp dx=0 位置 + ref_v=0 速度（STOP_DX 内线性生效）
export S10_SMppi_STOP_DX=3.0 S10_MPPI_W_TPOS=10.0 S10_MPPI_W_TV=10.0
# TMppi 原地转（四轮差速；防打滑/硬刹只在实际 vx>0.5 时启用）
export S10_TURN_SPLIT=1 S10_TURN_ERR_DEG=10 S10_TURN_K=3.0 S10_TURN_OM_MAX=1.5 S10_TURN_V_MAX=2.2 S10_WP_TURN_VX=0.2 S10_TURN_ARRIVE_R=0.6
...
export S10_STAIR_SINGLE_RISE=0.10 S10_ELEV_DROP_TH=0.10 S10_DROP_LOOKAHEAD=0.5 S10_DROP_OM_LOOKAHEAD=0.5 S10_DROP_VX=0.3 S10_ELEV_CLIMB_TH=0.2
...
export S10_POSTSTAIR_HOLD_DIST=0.9 S10_POSTSTAIR_HOLD_VX=1.0 S10_POSTSTAIR_HOLD_T=2.0 S10_TK2=1 S10_TK2_YAW_DB=0.25 S10_TK2_YAW_K=2.5 S10_TK2_YAW_MAX=1.5 S10_TK2_VX=1.2
export S10_STAIR_WHEEL_CLEAR=-0.02 S10_STAIR_REENTRY_GUARD=1.0 S10_STAIR_EXIT_VX=1.0

# CarVMC（抬轮前馈已删；WHEEL_D 回退 0.02）
export S10_Z_HIGH=1.0 S10_Z_TK1=1.1 S10_Z_TK2=1.15 S10_Z_PLAT=1.2 S10_Z_DROPW=1.3 S10_Z_DDE=1.3 S10_Z_DROP=1.5 S10_Z_WEAK=2.0
export S10_PLAT_VX=2.5  # 平台限速：5.0 实测走廊撞墙(round297 4m/s CTE摆振贴上y47.19墙基)；2.5 安全折中，近楼梯/下沿回退1.8
export S10_CAR_SQUAT=1
export S10_CAR_WHEEL_GF=1.0 S10_VMC_KPH=300 S10_VMC_KDH=60 S10_VMC_WHEEL_K=12.0 S10_VMC_WHEEL_D=0.02 S10_VMC_TERRAIN_LP=0.4 S10_VMC_TERRAIN_LOOKAHEAD=0.35 S10_VMC_TERRAIN_AHEAD_W=0.6 S10_CAR_KP_ROLL=150 S10_CAR_KD_ROLL=20 S10_CAR_ROLL_K=0 S10_VMC_Z_DES_OFFSET=0.26
