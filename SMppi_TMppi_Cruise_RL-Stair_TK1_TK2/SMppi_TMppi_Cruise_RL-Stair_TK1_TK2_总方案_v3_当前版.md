...
## 9. 当前链状态与断点（r58 终态，2026-08-20）

- **交付基线 = r37 链**（wp0→6 全通、全腿达标 27.5s：6.1/3.2/4.4/3.6/4.8/5.4s；
  验收 5s，>5m 段 8s；逐点可复现 r44/r47/r51 三验证）。
- **唯一断点：wp6→7 六级台阶顶台**：RL 登顶 z1.26 后 7m/s 飞跃翻滚（roll −2.05）。
  根因链：lidar 瓦片远单元欠填充（0.125 台阶在 5m 外跨 riser 掠射余量 1-3cm）→
  riser 表退化 2/6 级 → RL 出分布顶台步态暴力。
- **三条修复路线探明结论**：
  1. 交还动量阻尼：飞跃发生在 STAIR 模式内（vx 门恒不满足），交还后阻尼无效（实测排除）。
  2. 感知修复（桅杆 0.6→1.0m）+ 角部重调：**有效组合已探明**——桅杆1.0+判点0.7+
     TURN_V_MAX3.0+W_GUIDE3.0 → wp0-5 全腿达标 21.6s（r54），剩余 wp4 过 π 转弯
     2.9rad 过冲；转弯旋钮（K/KD/OM_MAX/V_MAX/ARRIVE_R）与五角部强耦合，
     单步进不收敛，需成对/网格调参或 yaw 结构修复（r55-r58 实测）。
  3. RL 观测补全：真 2 级台阶与退化 6 级在瓦片内无区分信号（待新思路）。
- **±π 结构脆弱性**：西向航段（3.119/2.91/1.67 rad）的转弯过 π，饱和 PD 下
  过冲达 2.9rad；r37 基线恰好避开，任何感知/旋钮扰动都会重新掷此分支。
- 下一会话承接点：路线2 组合（上述 4 旋钮）+ wp4 转弯成对调参，或用户指示
  的结构性 yaw 修复。
