...
make -j4
```
**[注意]** 程序默认接收手柄数据的端口号为12121，请确保程序的端口号没有被其他程序占用，如有需要，可打开/example目录下的example_retroid.cpp（对应Retroid手柄）或example_skydroid.cpp（对应绝影X30手柄）进行修改：
```c++
...
&nbsp;
## 2 程序运行
- 官方手柄在手柄上安装controlapp.apk，安装之后的界面如图所示。  

   <img src="./doc/app_icon.png" alt="a" style="zoom:60%;" />

- 手柄连接上开发主机的网络，然后打开app，点击左上角按钮，配置需要连接的开发主机的ip地址和程序接收手柄数据的端口号，图中以Retroid手柄为例。
