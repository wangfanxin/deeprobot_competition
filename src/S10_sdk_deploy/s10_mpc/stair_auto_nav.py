...
class AutoNavFollower:
    ...
    def _seg_in_stair_band(self, i):
        ...
        if len(risers) == 0:
            ...
            # v871: 无预扫描表时不用 z 先验兜底（终版：触发由感知确认负责）
            return False
    ...
    def update_mode(self, robot_xy, next_idx, yaw=None, local_map=None,
                    percept_confirmed=None, stair_exit=None):
        """技能状态机 CRUISE⇄STAIR（v871，终版计划）：
        - 入口：预扫描 riser 走廊带+距离窗（世界坐标，非 z 先验） OR
          lidar 高程图 step_flag 感知确认（percept_confirmed，主判据）；
        - 出口：几何完成（stair_exit，主循环由 stair_world 计算）AND
          导航推进（next 推进或 s_cur 过末级 riser+1.0m）；替换绝对 y；
        - 滞回：进入后保持，不因单帧感知失败退出。
        """
        if os.environ.get("S10_FORCE_MODE") in ("CRUISE", "STAIR"):
            ...
            return
        if self.mode == "STAIR":
            _adv = bool(next_idx > 7)
            _last_arc = float(getattr(self, "_stair_last_arc", 1e9))
            if not _adv:
                _adv = float(getattr(self, "_s_cur", 0.0)) > _last_arc + 1.0
            if bool(stair_exit) and _adv:
                self.mode = "CRUISE"
                if os.environ.get("S10_MODE_DEBUG"):
                    print("[MODE] CRUISE exit: geom+nav", flush=True)
            return
        _dbg = os.environ.get("S10_MODE_DEBUG")
        _d = None
        if next_idx >= 1 and next_idx < len(self.wp):
            ...
            _dseg1 = float(np.linalg.norm(
                robot_xy - self.wp[next_idx, :2]))
            # v871: 预扫描 riser 走廊带（世界坐标，无 z 先验）+ 距离窗
            _use_global = (self._seg_in_stair_band(next_idx - 1)
                           and _dseg1 <= float(os.environ.get(
                               "S10_STAIR_ENTER_DIST", "1.5")) + 2.5)
            if percept_confirmed is None:
                _pc = self._stair_confirmed(robot_xy, yaw, local_map)
            else:
                _pc = bool(percept_confirmed)
            _use_percept = (d_wp < _confirm_dist and _pc)
            if _use_global or _use_percept:
                self.mode = "STAIR"
                if _dbg:
                    print(f"[MODE] STAIR next={next_idx} "
                          f"d={d_wp:.1f} global={int(_use_global)} "
                          f"percept={int(_use_percept)}", flush=True)
                return
        if _dbg and int(robot_xy[1] * 2) % 10 == 0:
            print(f"[MODE] CRUISE next={next_idx} d={_d} "
