...
class AutoNavFollower:
    ...
    def _build_smooth_path(self):
        ...
        _cva = float(os.environ.get("S10_CURVE_VLIM_A", "3.0"))
        ...
        # USER-DIRECTED 2026-08-17: keep DECELERATION smoothing only; remove the
        # acceleration/start ramp so the path profile can accelerate immediately.
    ...
    def compute_cmd(self, robot_xy, yaw, next_idx, robot_z=None, yaw_rate=0.0,
        ...
        ref_yaw = float(np.arctan2(target[1] - robot_xy[1],
                                   target[0] - robot_xy[0]))
        err = float(np.arctan2(np.sin(ref_yaw - yaw), np.cos(ref_yaw - yaw)))
        self._last_err = err
        self._last_dwp = d_wp
