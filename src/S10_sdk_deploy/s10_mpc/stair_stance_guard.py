...
class StairStanceGuard:
    ...
    def apply(self, tau, gait_swing, com_xy, wheel_y=None, wheel_z=None, terrain_z=None, prox=None):
        ...
        if (request_swing[0] or request_swing[1]):
            # 下限强制：后轮扭矩 >= +boost（前进），压过 DiAL 的刹车/保持。
            # 前轮悬在棱前空隙时机器人必须前进让前轮滚过棱角（0.048<=R
            # 可滚）；DiAL 常停在保持位形局部极小（后轮差速/制动）。
            _rboost = float(os.environ.get('S10_FRONT_SWING_REAR_BOOST', '0.6'))
            for i in (2, 3):
                if contact[i]:
                    tau[self.wheel_act_ids[i]] = float(np.maximum(
                        tau[self.wheel_act_ids[i]],
                        _rboost * self.wheel_tau_max))
