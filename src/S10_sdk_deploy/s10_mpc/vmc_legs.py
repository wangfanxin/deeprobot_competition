...
class CarVMC:
    ...
    def compute_tau(self, qpos, qvel, wheel_xyz, wheel_vel,
        ...
        for leg in range(4):
            ...
            # v896: 用户指令恢复（v869 曾删除）；配 om 限幅(v889)防过冲
            ...
            # 摇振抑制（仅平顶，cmd.rock_kill=1）：|yaw率高频分量|
            # 持续大（2Hz 摆 ±2rad/s）时关反馈——反馈 bang-bang 饱和
            # ±13.5Nm 自持摇振、车身零位移（round223 wp10 后平顶实测）。
            # 用 0.3s 低通判持续（round227 瞬时阈值 1.0 误伤窄脊磕碰
            # 瞬态、掉下台沿侧翻）；单次瞬态不触发
            if not hasattr(self, "_rock_lp"):
                self._rock_lp = 0.0
            self._rock_lp += (abs(_om_hf) - self._rock_lp) * min(1.0, dt / 0.3)
            if (self._rock_lp > 1.0
